...
class _Vacuum {
  public:
    ...
    void update() {
      if (enabled) {
       ...
       // debugln("Vaccuum Pressure: ", vacuumPressure, "Torr");
        ...
        if (vacuumPressure > settings.minVacuumPressure) {        // if vacuum is not at a suitable level for high voltage to start
          if (!pumpRelayState) {  // if pump hasnt started yet
            debugln("Below min pressure, vacuum pump started");
            pumpRelayState = 1;
            pumpStartTime = timeNow;
          } 
          else if (timeNow > pumpStartTime + settings.vacuumPumpTimeout) {       // if pump has been on longer than timeout
           // Alarm.raise(VACUUM_LEAK_DETECTED); // Pump has been not achived vacuum within timeout period error
           // pumpRelayState = 0;
          }
        }
        else if (vacuumPressure < settings.maxVacuumPressure) {
          if (pumpRelayState) {
    ...
    bool isReady() {
      return vacuumPressure < settings.minVacuumPressure;
    }
