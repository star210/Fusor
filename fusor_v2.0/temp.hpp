...
class _Temp {
  public:
    ...
    void begin() {
      sensors.begin();
      debugln("Found", sensors.getDeviceCount(), "temperature sensors");
    }

    void update() {
      auto timeNow = millis();
      if (timeNow > lastUpdateTime + settings.updateInterval) {
      ...
      if (pumpTemp > settings.maxPumpTemp) {
        Alarm.raise(VACUUM_PUMP_OVERHEAT);
      }
      if (plateTemp > settings.maxPlasmaTemp) {
        Alarm.raise(PLASMA_PLATE_OVERHEAT);
      }
      if (transformerTemp > settings.maxTransformerTemp) {
